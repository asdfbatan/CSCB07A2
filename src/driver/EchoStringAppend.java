 ...
 */
public class EchoStringAppend extends Commands {

  /**
   * constructor for EchoStringAppend
   */
  public EchoStringAppend() {}


  /**
   * This method takes in 2 inputs: text and path. text: the content. path: can
   * be a path or a file name. If it's a path, seperate them to 2: directory
   * path and file name. Pass on the path, file name and content to the
   * runCommandHelper.If it's a file name, pass on the current path, file name
   * and content to the runCommandHelper.
   * 
  ...
  public String runCommand(String inputText, String inputPath)
    ...
    for (File f : Directory.getCurrDir().getFiles()) {
      // if OUTFILE exist,
      // append to a new file in the file.
  ...
  public static void main(String[] args) {
    ...
    Cd cd = new Cd();
    cd.runCommand("/A/C");
    EchoStringAppend esa = new EchoStringAppend();
    try {
      esa.runCommand("hello", "/A/B/newFILE");
    } catch (Exception e) {
